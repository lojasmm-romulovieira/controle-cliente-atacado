...
export default function TableFlowbite(props) {
    const {columns, pagination, rows, filters, hideSearch, title, subtitle} = props;
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                    ...
                    <tr>
                        {columns.map((column, index) => (
                            <th key={index} scope="col" className="px-6 py-3" {...getColumnCustomStyles(column)}>
                                {column.label}
                            </th>
                        ))}
                    </tr>
                    ...
                    {rows.map((row, index) => (
                        <tr key={index}
                            ...
                            {columns.map((column, index) => {
                                ...
                                return (
                                    <td key={index}
                                        className={"px-6 py-2 " + colorColumn} {...getColumnCustomStyles(column)}>
                                        {row[column?.accessor]}
                                    </td>
                                )
                            })}
                    ...
                    {rows.length === 0 && (
                        <tr>
                            <td colSpan={columns.length} className="text-center py-4">Nenhum registro encontrado</td>
                        </tr>
                    )}
                    </tfoot>
                </table>
